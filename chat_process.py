...
def msg_filter_basic(chat_text):
    filtered = []
    ...
    for msg in msgs:
        ...
        deleted_msg_filter = ("This message was deleted" in line or "You deleted this message" in line or ("<This "
                                                                                                           "message "
                                                                                                           "was "
                                                                                                           "edited>")
                              in line or "(file attached)" in line)
...
class Wh_Chat_Processor:
    ...
    def open_chat_file(self, directory, filename):
    ...
    def advance_filter(self, merged_chat_data):
        ...
        for ind, x in enumerate(CD):
            if x.get(self.sender_name) is not None:
                if len(x[self.sender_name]) == 1 and (
                        x[self.sender_name][0] in filler_words or len(x[self.sender_name][0]) == 1):
                    continue
            ...
            elif x.get(self.my_name) is not None and len(sender) > 1:
...
for file in os.listdir(os.path.join(chat_dir)):
    ...
    if file.endswith('.txt'):
        ...
        basic_f = msg_filter_basic(chat_d)
